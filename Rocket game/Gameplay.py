...
def key_movement():
    global moveU, moveL, moveR, moveD, player, colors
    glPushMatrix()
    r, g, b = colors[colorIndex]
    distance = 25
    player.draw_rect(r, g, b, names[2], 1, 1)
    if start:
        if moveU:
            if player.top + distance <= WINDOW_HEIGHT * 10:
                player.top += distance
                player.bottom += distance
            player.draw_rect(r, g, b, names[2], 1, 1)
            moveU = False
        elif moveR:
            if player.right + distance <= WINDOW_WIDTH:
                player.right += distance
                player.left += distance
            player.draw_rect(r, g, b, names[2], 1, 1)
            moveR = False
        elif moveL:
            if player.left - distance >= 0:
                player.left -= distance
                player.right -= distance
            player.draw_rect(r, g, b, names[2], 1, 1)
            moveL = False
        elif moveD:
            if player.bottom - distance >= TOP_GROUND:
                player.bottom -= distance
                player.top -= distance
            player.draw_rect(r, g, b, names[2], 1, 1)
            moveD = False
...
def mouse(x, y):
    global current_x, current_y, displace
    current_x = x
    current_y = y
    current_y = -current_y + WINDOW_HEIGHT - speed
    displace = current_y + abs(displacement_amount)


def keyboard(key, xx, yy):
    global moveU, moveL, moveR, moveD, start, gameOver, winning, store
    if key == b'w' and start is True:
        moveU = True
        moveR = False
        moveL = False
        moveD = False
    elif key == b'a' and start is True:
        moveR = False
        moveL = True
        moveU = False
        moveD = False
    elif key == b'd' and start is True:
        moveR = True
        moveL = False
        moveU = False
        moveD = False
        ...
        start = True
    elif key == b's' and start is True:
        moveR = False
        moveL = False
        moveU = False
        moveD = True
...
# first condition for keyboard movement
# second condition for mouse movement
